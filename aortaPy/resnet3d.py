...
def _shortcut3d(input, residual):
    ...
    stride_dim1 = ceil(input._shape_val[DIM1_AXIS] \
        / residual._shape_val[DIM1_AXIS])
    stride_dim2 = ceil(input._shape_val[DIM2_AXIS] \
        / residual._shape_val[DIM2_AXIS])
    stride_dim3 = ceil(input._shape_val[DIM3_AXIS] \
        / residual._shape_val[DIM3_AXIS])
    equal_channels = residual._shape_val[CHANNEL_AXIS] \
        == input._shape_val[CHANNEL_AXIS]

    shortcut = input
    if stride_dim1 > 1 or stride_dim2 > 1 or stride_dim3 > 1 \
            or not equal_channels:
        shortcut = Conv3D(
            filters=residual._shape_val[CHANNEL_AXIS],
            kernel_size=(1, 1, 1),
...
class Resnet3DBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions, reg_factor):
        ...
        pool2 = AveragePooling3D(pool_size=(block._shape_val[DIM1_AXIS],
                                            block._shape_val[DIM2_AXIS],
                                            block._shape_val[DIM3_AXIS]),
